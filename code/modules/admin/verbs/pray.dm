...
/proc/high_command_announce(text , mob/Sender , iamessage)
	...
	msg = "<b>[SPAN_STAFF_IC("<font color=orange>UACM[iamessage ?  "IA" : ""]:</font>")][key_name(Sender, 1)] [CC_MARK(Sender)] [ADMIN_PP(Sender)] [ADMIN_VV(Sender)] [ADMIN_SM(Sender)] [ADMIN_JMP_USER(Sender)] [CC_REPLY(Sender)]:</b> [msg]"
	log_admin(msg)
	for(var/client/C in GLOB.admins)
